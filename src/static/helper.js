...
function handleFile(event) {
  ...
  reader.onload = () => {
    try {
      ...
      syncRMSSDChart(CHART_MAX_POINTS-300)
    } catch (err) {
      alert(err.message);
    }
  };
...
function updateChartWidth(){
  ...
  let new_max = Math.max(Math.round(last.time+10), CHART_MAX_POINTS)
  console.log(new_max)
  chart.options.scales.x.max = new_max;
  chartContainer.style.width = new_max * WIDTH_PER_POINT + 'px';
  chart.canvas.style.width = new_max * WIDTH_PER_POINT + 'px';
